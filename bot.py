...
async def on_ready():
    ...
    try:
        # Sync Command ตรงเข้า GuildWhilist เพื่อป้องกันปัญหาสั่งงานค้าง หรือ CommandNotFound
        guild_obj = discord.Object(id=CONFIG["ALLOWED_GUILDS"][0])
        bot.tree.clear_commands(guild=guild_obj)
        bot.tree.copy_global_to(guild=guild_obj)
        synced = await bot.tree.sync(guild=guild_obj)
        logging.info(f"✅ Sync Slash Commands สำเร็จ! ทั้งหมด {len(synced)} คำสั่ง")
    except Exception as e:
        logging.error(f"❌ เกิดข้อผิดพลาดในการ Sync คำสั่ง: {e}")

# =============================================================
# 💻 SLASH COMMANDS (รวมคำสั่ง ไม่ให้ซ้ำซ้อน)
# =============================================================
