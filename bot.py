...
async def on_ready():
    ...
    try:
        ...
        # ลบคำสั่งเก่าที่ค้างในระบบออกทั้งหมด แล้ว Sync คำสั่งชุดใหม่เข้าไป
...
# --- SLASH COMMANDS (PERMISSION LOCKED TO 933529869487321161) ---
...
async def setup_panel(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("กำลังสร้าง Panel...", ephemeral=True)
    await interaction.channel.send(embed=embed, view=RegistrationView())
    await interaction.edit_original_response(content="✅ สร้าง Panel เรียบร้อยแล้ว")
...
@bot.tree.command(name="set-default-voice", description="ตั้งค่าห้องเสียงเริ่มต้นสำหรับผู้เล่น (Admin Only)")
...
async def set_default_voice(interaction: discord.Interaction, channel: discord.VoiceChannel):
    ...
    db["default_voice_channel_id"] = channel.id
    save_database()
    await interaction.response.send_message(f"✅ ตั้งค่าห้องเสียงเริ่มต้นเป็น **{channel.name}** เรียบร้อยแล้ว", ephemeral=True)
...
@bot.tree.command(name="voice-status", description="ตรวจสอบสถานะการเชื่อมต่อระบบเสียง (Admin Only)")
async def voice_status(interaction: discord.Interaction):
    if not is_authorized_admin(interaction):
        await interaction.response.send_message("❌ คุณไม่มีสิทธิ์ใช้งานคำสั่งนี้", ephemeral=True)
        return

    conn_count = len(minecraft_connections)
    registered_count = len(db.get("user_database", {}))
    
    embed = discord.Embed(title="📊 สถานะระบบ Minecraft Voice Chat", color=discord.Color.green())
    embed.add_field(name="🔌 Minecraft Connections", value=f"`{conn_count}` เครื่อง", inline=True)
    embed.add_field(name="👥 ผู้ลงทะเบียนทั้งหมด", value=f"`{registered_count}` คน", inline=True)
    embed.add_field(name="🚨 SOS Channel", value=f"<#{db.get('sos_channel_id', 0)}>" if db.get('sos_channel_id') else "`ยังไม่ได้ตั้งค่า`", inline=False)
