...
async def on_ready():
    ...
    for guild in bot.guilds:
        try:
            ...
            logging.info(f"⚡ Instant Sync สำเร็จในเซิร์ฟเวอร์ {guild.name} ({len(synced)} คำสั่ง)")
        except Exception as e:
            logging.error(f"❌ Error Syncing to guild {guild.id}: {e}")
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    # ลบห้องว่างอัตโนมัติเมื่อออกจากห้องที่บอทสร้าง
...
async def handle_proximity_routing(guild_id: int, player_list: list):
    ...
    for i in range(len(sessions)):
        for j in range(i + 1, len(sessions)):
            ...
            if not p2_member or not p2_member.voice: continue
    ...
    try:
        ...
        if msg_type == "PLAYER_JOIN":
            ...
            if d_id:
                ...
                if member:
                    try:
                        # แบน/ปลดแบนการส่งเสียงในห้องเสียง Discord
