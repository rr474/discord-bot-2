        ...
        if msg_type == "START_CALL" and guild:
            ...
            for x_name in targets:
                ...
                if d_id:
                    ...
                    # เช็คว่าผู้เล่นอยู่ในห้อง Lobby เริ่มต้นหรือไม่ (ถ้าตั้งค่า Lobby ไว้)
                    if m and m.voice:
                        if default_lobby_channel_id and m.voice.channel and m.voice.channel.id == default_lobby_channel_id:
                            discord_members.append(m)
                        elif not default_lobby_channel_id: # ถ้าไม่ได้ตั้ง Lobby ไว้ จะดึงหมดทุกคนที่อยู่ในห้องเสียงไหนก็ได้
                            discord_members.append(m)
            
            # ถ้ามีคนอยู่ใน Lobby ตามเงื่อนไข ถึงจะสร้างห้องใหม่
            if discord_members:
                ...
                # ย้ายทุกคนจากห้อง Lobby มาห้องใหม่
...
@bot.tree.command(name="set-default-voice", description="ตั้งค่าห้อง Lobby เริ่มต้นที่ผู้เล่นต้องเข้ามานั่งรอ")
