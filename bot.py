    ...
    try:
        ...
        if msg_type == "POSITIONS_UPDATE" and guild:
            ...
            for d_id, info in player_coords.items():
                ...
                # ถ้าอยู่คนเดียว ไม่ได้ใกล้ใคร -> สร้างห้องเดี่ยวให้เล่นคนเดียว
                if len(group) == 1:
                    ...
                    # ถ้าอยู่ใกล้กัน ดึงมารวมห้องเดียวกัน
            ...
            # เช็คคนที่ออกจากเกม -> ลบห้องเก่าทิ้ง และส่งกลับห้อง Lobby เริ่มต้น
...
class RegisterModal(discord.ui.Modal, title="📝 ลงทะเบียนข้อมูลผู้เล่น"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=f"ยินดีต้อนรับคุณ {interaction.user.mention} เข้าสู่ระบบ!\n\n• **Xbox:** `{self.xbox_name.value}`\n• **ชื่อ IC:** `{i_name}`",
...
class PanelView(discord.ui.View):
    ...
    @discord.ui.button(label="ลงทะเบียนชื่อ (Xbox & IC)", style=discord.ButtonStyle.green, custom_id="btn_register", emoji="📝")
...
@bot.tree.command(name="setup-panel", description="สร้าง Panel ปุ่มกดลงทะเบียนสวยๆ")
async def setup_panel(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🎙️ ระบบ Proximity Voice & ลงทะเบียนเซิร์ฟเวอร์", 
        description="กดปุ่มด้านล่างเพื่อทำการลงทะเบียนชื่อ Xbox และชื่อ IC ของคุณสำหรับใช้งานระบบเสียงและโทรศัพท์ในเกม", 
        color=discord.Color.from_rgb(88, 101, 242)
    )
    embed.set_footer(text="Memory Shop Roleplay System")
    await interaction.channel.send(embed=embed, view=PanelView())
    await interaction.response.send_message("✅ สร้าง Panel เรียบร้อยแล้ว!", ephemeral=True)

@bot.tree.command(name="set-category", description="เลือกหมวดหมู่ที่บอทจะใช้สร้างห้องเกม")
async def set_category(interaction: discord.Interaction, category: discord.CategoryChannel):
    global default_category_id
    default_category_id = category.id
    await interaction.response.send_message(f"✅ ตั้งค่าหมวดหมู่ห้องสร้างเกมเป็น: **{category.name}** เรียบร้อย", ephemeral=True)

@bot.tree.command(name="set-lobby", description="เลือกห้องล็อบบี้เริ่มต้นสำหรับรอเข้าเกม")
...
@bot.tree.command(name="set-sos-role", description="เลือกแท็กยศแจ้งเตือน SOS")
