...
# 🚨 ตั้งค่าช่องแจ้งเหตุ SOS พร้อมเลือก Tag/Role ที่ต้องการให้แจ้งเตือน
@bot.tree.command(name="set-sos-channel", description="ตั้งค่าช่องแจ้งเตือนเหตุฉุกเฉิน (SOS) และเลือก Role ที่ต้องการแท็ก")
async def set_sos_channel(interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role = None):
    ...
    msg = f"✅ ตั้งค่าช่องแจ้งเตือน SOS เป็น: {channel.mention}"
    if role:
        msg += f"\n🏷️ แท็กยศ/Role เมื่อมีเหตุ: {role.mention}"
    else:
        msg += f"\n🏷️ แท็กยศ/Role: `@here` (ไม่ได้ระบุ Role เจาะจง)"
