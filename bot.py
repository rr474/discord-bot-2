...
async def on_ready():
    ...
    try:
        # 1. ล้างคำสั่ง Global เก่าออก ป้องกันคำสั่งซ้ำในเมนู Discord
        bot.tree.clear_commands(guild=None)
        await bot.tree.sync(guild=None)

        # 2. Sync คำสั่งระดับ Guild เฉพาะตาม Whitelist
        for guild_id in CONFIG["ALLOWED_GUILDS"]:
            guild_obj = discord.Object(id=guild_id)
            bot.tree.copy_global_to(guild=guild_obj)
            synced = await bot.tree.sync(guild=guild_obj)
            logging.info(f"✅ Sync Slash Commands เข้า Guild {guild_id} สำเร็จ! ทั้งหมด {len(synced)} คำสั่ง")
...
async def setup_panel(interaction: discord.Interaction):
    ...
    # Defer การตอบรับเพื่อป้องกัน Timeout
    ...
    try:
        ...
        await interaction.followup.send("❌ บอทไม่มีสิทธิ์ส่งข้อความในช่องนี้ โปรดเช็กสิทธิ์ Send Messages / Embed Links", ephemeral=True)
        return
