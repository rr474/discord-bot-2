...
async def handle_mc_update(request):
    ...
    try:
        ...
        if msg_type == "START_CALL" and guild:
            ...
            # สร้างห้องใน Category ที่ตั้งค่าไว้ (ถ้ามี)
...
async def set_sos_role(interaction: discord.Interaction, role: discord.Role):
    global sos_role_id
    sos_role_id = role.id
    await interaction.response.send_message(f"✅ ตั้ง่ายยศแจ้งเตือน SOS เป็น: {role.mention}", ephemeral=True)
...
@bot.event
async def on_ready():
    bot.add_view(PanelView()) # ทำให้ปุ่มใช้งานได้ตลอดแม้บอทรีสตาร์ท
    await bot.tree.sync()
