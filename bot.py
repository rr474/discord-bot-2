...
proximity_radius = 15   # ระยะเริ่มต้น (บล็อก)
...
async def on_ready():
    ...
    try:
        ...
        logging.info(f"✅ Auto-Synced Commands ทั้งหมด {len(synced)} คำสั่ง")
    except Exception as e:
        logging.error(f"❌ Error Auto-Syncing Commands: {e}")
    ...
    if before.channel and before.channel != after.channel:
        ...
        if is_bot_created and len(old_channel.members) == 0:
            try:
                ...
                logging.error(f"❌ Error deleting channel {old_channel.name}: {e}")

# =============================================================
# 💻 SLASH COMMANDS (ใช้ / ทั้งหมด)
# =============================================================

# 🔄 คำสั่ง Slash Command เพื่อรีเฟรช / ซิงค์รายการคำสั่งเข้า Discord
@bot.tree.command(name="sync", description="อัปเดตและบังคับซิงค์รายการคำสั่ง Slash ทั้งหมด")
async def sync_slash(interaction: discord.Interaction):
    ...
    try:
        synced = await bot.tree.sync()
        await interaction.followup.send(f"✅ บังคับซิงค์คำสั่งสำเร็จ! มีทั้งหมด {len(synced)} คำสั่ง (หากยังไม่ขึ้น กรุณาปิด-เปิดแอป Discord ใหม่)", ephemeral=True)
    except Exception as e:
        await interaction.followup.send(f"❌ เกิดข้อผิดพลาดในการซิงค์: {e}", ephemeral=True)

# 🚨 คำสั่งเลือกช่องและเลือกยศ/Role แจ้งเตือน SOS
@bot.tree.command(name="set-sos-channel", description="ตั้งค่าช่องแจ้งเตือนเหตุฉุกเฉิน (SOS) และเลือกยศ/Role ที่จะแท็ก")
@app_commands.describe(channel="เลือกช่องข้อความสำหรับรับแจ้งเตือน", role="เลือกยศ/Role ที่ต้องการให้แท็ก (ไม่ใส่จะใช้ @here)")
async def set_sos_channel(interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role = None):
    ...
    if role:
        msg += f"\n- **ยศ/Role ที่จะแท็ก:** {role.mention}"
    else:
        msg += f"\n- **ยศ/Role ที่จะแท็ก:** `@here`"
